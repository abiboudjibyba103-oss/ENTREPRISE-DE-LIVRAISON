...
-- Lets generate-predictions store up to 4 "mémoires personnalisées" per
-- user per day, regenerable same-day without piling up duplicates
-- (mirrors daily_lessons / coach_messages' own "one per day" pattern).
...
-- ------------------------------------------------------------
-- 9. paydunya_payments — one row per PayDunya invoice the
--    paydunya-webhook edge function has actually confirmed as paid.
--    unique(invoice_token) is what makes the webhook idempotent: a
--    retried IPN call (PayDunya resends until it gets a 200) can only
--    insert once, so profiles.plan_expire_at is never pushed forward
